...
$ARCH  = "$ROOT\fpga\ai_suite\resnet8_agx3_int8_k16c16_ocp.arch"
$IR    = "$ROOT\build\fpga_ai\openvino_ir\resnet8_fpga_logits.xml"
$HET   = "$ROOT\build\fpga_ai\compile_k16c16_ocp"
...
Run-Ai ("dla_compiler --march `"$ARCH`" --network-file `"$IR`" " +
        ...
        "--o resnet8_k16c16_ocp.aot --dumpdir `"$HET`" --overwrite-output-files " +
        "--fanalyze-performance --festimate-per-layer-latencies --fanalyze-area " +
        "--fdump-area-report area-report.txt") "$ROOT\build\fpga_ai\k16c16_ocp_hetero.log"
...
Run-Ai ("dla_create_ip --skip-sim-env --overwrite --ip-dir `"$IPDIR`" " +
       ...
       "$ROOT\build\fpga_ai\k16c16_ocp_createip.log"
